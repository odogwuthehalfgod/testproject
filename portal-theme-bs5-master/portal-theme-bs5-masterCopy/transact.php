...
while($row=$sql->fetch(PDO::FETCH_ASSOC)){
   if($final_level == $row["final_level"]){
    ...
    echo "<a href='transact.php?id=".$row["id"]."'>Accept Goods</a><br>";
}else{
...
$select_inv = $pdo->prepare("select * from inventory where store_id=:ddd and product_id=:fff");


 $insert = $pdo->prepare("insert into inventory (store_id,product_id,qty) values (:store_id,:product_id,:qty)");
...
for($i=0;$i<$num_of_rows;$i++){
    ...
    $select_inv->bindParam(":ddd", $row1[$i]["store_to"]);
    $select_inv->bindParam(":fff", $row1[$i]["product__id"]);

    $select_inv->execute();

   
    if($select_inv->rowCount() > 0){
        echo "check here";
        $update = $pdo->prepare("update inventory set qty=qty+".$row1[$i]["quantity"]." where store_id=:ggg and product_id=:hhh");

        $update->bindParam(":ggg", $row1[$i]["store_to"]);
        $update->bindParam(":hhh", $row1[$i]["product__id"]);
        ...
        if($update->rowCount()){
            echo "This item quantity has been updated";
        }
    }

    if($select_inv->rowCount() == 0){
         $insert->bindParam(":store_id", $row1[$i]["store_to"]);
        $insert->bindParam(":product_id", $row1[$i]["product__id"]);
        $insert->bindParam(":qty", $row1[$i]["quantity"]);
